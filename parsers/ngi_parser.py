        ...
        try:
            ...
            for i in range(num_points):
                ...
                if (
                    not line
                    or line.startswith("$")
                    or line in ["POLYGON", "LINESTRING", "POINT"]
                ):
                    break
                try:
            ...
            while i < len(lines):
                ...
                if current_record and current_layer:
                    if line == "POLYGON":
                        ...
                        if len(coords) < 4:
                            logger.warning(
                                f"Layer {current_layer}, Record {current_record}: Polygon has less than 4 coordinates"
                            )
                            i += 1
                            continue
    ...
    def get_layer_definition(self, layer_name: str) -> LayerDefinition:
        ...
        if layer_name not in geometry_types:
            return LayerDefinition(
                name=layer_name, fields=base_fields, geometry_type=GeometryType.UNKNOWN
            )
